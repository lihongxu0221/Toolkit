...
public sealed class SemanticVersion : IComparable, IComparable<SemanticVersion>, IEquatable<SemanticVersion>
{
    ...
    {
        Major = version.Major == -1 ? 0 : version.Major;
    ...
    /// <summary>
    /// Gets PreReleaseLabel position in the SymVer string 'major.minor.patch-PreReleaseLabel+BuildLabel'.
    /// </summary>
    public string? PreReleaseLabel { get; }

    /// <summary>
    /// Gets BuildLabel position in the SymVer string 'major.minor.patch-PreReleaseLabel+BuildLabel'.
    /// </summary>
    public string? BuildLabel { get; }

    /// <summary>
    /// Gets a value indicating whether whether the version is a prerelease.
    /// </summary>
    ...
    {
        ArgumentNullException.ThrowIfNullOrEmpty(version, nameof(version));

        var r = default(VersionResult);
        r.Init(true);
        _ = TryParseVersion(version, ref r);
    ...
    public static bool TryParse(string? version, [NotNullWhen(true)] out SemanticVersion? result)
    {
        if (version != null)
        {
            var r = default(VersionResult);
