...
{
    public static readonly DependencyProperty XProperty = DependencyProperty.Register(nameof(X), typeof(double), typeof(MouseDragElementBehaviorEx), new PropertyMetadata(double.NaN, OnXChanged));

    public static readonly DependencyProperty YProperty = DependencyProperty.Register(nameof(Y), typeof(double), typeof(MouseDragElementBehaviorEx), new PropertyMetadata(double.NaN, OnYChanged));

    public static readonly DependencyProperty ConstrainToParentBoundsProperty = DependencyProperty.Register(nameof(ConstrainToParentBounds), typeof(bool), typeof(MouseDragElementBehaviorEx), new PropertyMetadata(ValueBoxes.FalseBox, OnConstrainToParentBoundsChanged));

    private Transform? _cachedRenderTransform;
    private Point _relativePosition = new Point(0, 0);
    private bool _settingPosition;

    /// <summary>
    ///  Gets or sets a value indicating whether 是否固定住Y轴
    /// </summary>
    public bool LockY { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether 是否固定住X轴
    /// </summary>
    ...
    private void UpdatePosition(Point point)
    {
        ...
        Point transformOffset = GetTransformOffset(AssociatedObject.TransformToVisual(RootElement));
        ApplyTranslation(double.IsNaN(point.X) ? 0.0 : point.X - transformOffset.X, double.IsNaN(point.Y) ? 0.0 : point.Y - transformOffset.Y);
    }
    ...
    private void ApplyTranslation(double x, double y)
    {
        ...
        if (ConstrainToParentBounds)
        {
            ...
            Rect rect2 = AssociatedObject.TransformToVisual(parentElement).TransformBounds(ElementBounds);
            rect2.X += x;
            rect2.Y += y;
    ...
    {
        Point point = TransformAsVector(AssociatedObject.TransformToVisual(RootElement),
