...
/// 用于显示错误的视图模型。
/// </summary>
public partial class ErrorViewModel : NavigableDialogViewModel
{
    /// <summary>
    ...
    {
        ArgumentNullException.ThrowIfNull(navigationContext, nameof(navigationContext));

        Exception? ex = navigationContext.Parameters.GetValue<Exception>("error");
        if (ex != null)
        {
            this.ErrorMessage = ex.Message;
            this.ErrorDetails = ex.ToString();
        ...
        Action? retryAction = navigationContext.Parameters.GetValue<Action>("retryAction");
        if (retryAction != null)
        {
            this.RetryCommand = new DelegateCommand(retryAction);
            this.HasRetryAction = true;
