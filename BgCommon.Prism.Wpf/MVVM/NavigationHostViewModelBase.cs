...
public abstract partial class NavigationHostViewModelBase : NavigationViewModelBase, IInitialize, IScopedRegionManagerAware
{
    ...
    protected override Task<bool> OnConfirmNavigationRequestAsync(NavigationContext navigationContext)
    {
        ...
        ModuleInfo module = navigationContext.Parameters.GetValue<ModuleInfo>("CurrentView");
        if (module != null)
        {
            // 在这里执行权限验证！
            if (!this.moduleService.Verify(module))
            {
                // 权限不足
                canNavigate = false;
    ...
    public void Initialize(INavigationParameters? parameters = null)
    {
        ...
        _ = Application.Current.Dispatcher.InvokeAsync(() =>
        {
            ...
            // 如果没有指定父模块ID，则使用默认的0.
            int parentId = this.DataContext?.ModuleId ?? 0;
    ...
    private void OnModuleHostViewLoaded(ModuleHostViewChanged e)
    {
        if (e.Module != null && this.Views.Contains(e.Module))
        {
            ...
            {
                // 处理异常情况.
                LogRun.Error($"模块 '{e.Module.ModuleName}' 在 ModuleHostView 内部加载时发生异常: {e.Exception}");
    ...
    /// 执行实际的导航逻辑.
    /// </summary>
    private async Task NavigateToViewAsync(ModuleInfo module, ModuleInfo? previousModule)
    {
        Trace.WriteLine($"【{module.ModuleName}】NavigateToViewAsync");
        this.IsNavigating = true;
        string viewNameLocalized = this.GetString(module.ModuleName);

        try
        {
            // 准备导航参数.
            Type? viewType = module.GetModuleType() ?? typeof(DefaultView);
            string targetName = viewType.Name;
            NavigationParameters keys = new NavigationParameters()
            {
                { Constraints.CurrentView, module }
            };
                ...
                result =>
                {
                    ...
                    else
                    {
                        ...
                        else if (result.Cancelled)
                        {
                            ...
                            this.selectedView = previousModule;
